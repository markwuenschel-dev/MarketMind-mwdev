"""Integration test for the OI-59 Experiment 4 episode-construction CLI."""
...
def test_run_oi59_episode_construction_emits_required_artifacts(
    tmp_path: Path, deterministic_seed: int
) -> None:
    _ = deterministic_seed
    from scripts.run_oi59_episode_construction import run_oi59_episode_construction
    ...
    report = run_oi59_episode_construction(
    ...
    assert (tmp_path / "out" / "oi59_episode_construction_report.json").exists()
    assert (tmp_path / "out" / "oi59_episode_construction_diagnostic.md").exists()
    ...
    assert written["selected_baseline_variant"] == "B_regime_class_compatible_merge"
    assert "decision_output" in written
